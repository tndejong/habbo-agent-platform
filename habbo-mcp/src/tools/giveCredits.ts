...
export async function giveCredits(params: {
  username: string;
  amount: number;
  ...
  const response = await sendRconCommand('givecredits', { user_id: userId, credits: amount });

  if (response.status !== 0) {
    return { success: false, message: response.message || 'RCON error' };
  }
  return { success: true, message: `Gave ${amount} credits to ${username}` };
}
